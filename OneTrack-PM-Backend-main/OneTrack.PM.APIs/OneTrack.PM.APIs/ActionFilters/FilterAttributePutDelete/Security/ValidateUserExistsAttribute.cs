...
namespace OneTrack.PM.APIs.ActionFilters.FilterAttributePutDelete.Security
{
    public class ValidateUserExistsAttribute : IAsyncActionFilter
    {
        ...
        public ValidateUserExistsAttribute(IUnitOfWork unitOfWork, ILoggerService logger)
        {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            var entity = await _unitOfWork.Repository<SecUser>().GetWithSpecAsync(new UsersSpecifications(id), trackChanges);
            if (entity == null)
            {
                string msg = $"User with id: {id} doesn't exist in the database.";
                _logger.LogInfo(msg);
                context.Result = new NotFoundObjectResult(new ApiResponse(404, msg));
            }
            else
            {
                context.HttpContext.Items.Add("User", entity);
                await next();
            }
        }
    }
